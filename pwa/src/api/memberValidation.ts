 ...
 * in `buildMemberNameMap` — a parent-level `useMemo` in `pages/BorrowPage.tsx`
 * that runs before any card mounts — and React throws outright when an
 * object-valued `displayName` is rendered as a child (`components/MemberList.tsx`).
 * Neither end has an ErrorBoundary, so a single malformed element takes the
 * whole page down.
 *
 ...
 *
 * Kept in sync with `extension/src/api/memberValidation.ts` — the two ends
 * deliberately stay separate copies (no `shared/` module), mirroring the
 * PR #132 convention.
 *
 * `BoolFlag` comes from `./client`, which imports this module back, so this one
 * import cycle is NOT erased at compile time (the enum is a runtime value; the
 * project's Boolean Convention rules out comparing against raw `0` / `1`
 * instead). It is safe because neither side touches the other during module
 * evaluation — `BoolFlag` is read inside a function body here, and this module's
 * export is called inside a method body there — so the binding is initialised by
 * the time it is used, whichever module is entered first.
 */

import { BoolFlag } from "./client";
import type { ApiResponse, FamilyGroup, FamilyMember } from "./client";
 ...
 * (`canLend !== BoolFlag.FALSE` in `components/MemberList.tsx`), which is the
...
export function sanitizeFamilyMembersResponse(
  ...
  return {
    ...
    data: {
      ...
      // and this end publishes the same value through `hooks/useFamilyData.tsx`
      // for any screen to render, so an object-valued claim must not survive
      // here either. Any string survives verbatim; everything else collapses to
