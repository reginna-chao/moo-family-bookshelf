...
const LIBRARY_SELECTORS = [
  ...
  {
    selector: ".library-item .info .title[title]",
    description: "書名（title 屬性）",
    required: true,
  },
  {
    selector: ".library-item .cover-img[src]",
    description: "封面圖片",
    required: true,
  },
  {
    selector: ".library-item .openbook",
    description: "hover 後的操作區",
    required: true,
  },
  {
    selector: ".library-item .openbook a.reader-link[href]",
    description: "閱讀連結（含 bookId）",
    required: true,
  },
  {
    selector: ".library-item .privacy[id^='privacy-']",
    description: "fallback bookId",
    required: false,
  },
  {
    selector: ".desktop-top-nav-btn",
    description: "頂部導覽列按鈕",
    required: false,
  },
] as const;
...
const ME_SELECTORS = [
  ...
  {
    selector: ".me-panel div[style*='font-size: 16px']",
    description: "顯示名稱",
    required: true,
  },
] as const;
...
async function main() {
  ...
  try {
    ...
    await page.goto(READMOO_LIBRARY, {
    ...
    let foundBooks = await libraryItem
    ...
    if (!foundBooks) {
      ...
      console.log(
        "   （登入後請導航到書櫃頁面：read.readmoo.com/#/library）\n",
      );
    ...
    for (const { selector, description, required } of LIBRARY_SELECTORS) {
      ...
      console.log(
        `  ${icon} ${selector} — ${description} ${countStr}${sampleStr}`,
      );
    ...
    if (updateMock) {
      ...
      const borrowedCount = await page
        .locator('.library-item [type="borrowed"]')
        .count();
    ...
    console.log(
      `  ${emailIcon} Email 偵測邏輯 — div[style] 含 @ 和 .${emailFound ? ` → "${emailFound}"` : ""}`,
    );
