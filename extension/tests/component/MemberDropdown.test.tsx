...
const MEMBERS = [
  { userId: "user-self", displayName: "Me", books: [{ bookId: "s1" }, { bookId: "s2" }] },
  { userId: "user-a", displayName: "Alice", books: [{ bookId: "b1" }] },
  { userId: "user-b", displayName: "Bob", books: [{ bookId: "b2" }, { bookId: "b3" }] },
...
describe("MemberDropdown", () => {
  ...
  it("reflects a member value on the trigger when that member is selected", () => {
    ...
    expect(screen.getByRole("button", { name: "篩選成員" })).toHaveTextContent("Alice");
  });
  ...
  describe("option counts (totals, including hidden)", () => {
    ...
    const cases: Array<{ name: string; optionLabel: RegExp; expected: string }> = [
      { name: "all sums every member's books", optionLabel: /所有人的書/, expected: "5" },
      { name: "all-except-self sums non-self members", optionLabel: /其他家人的書/, expected: "3" },
      { name: "self shows the self member's book count", optionLabel: /自己的書/, expected: "2" },
      { name: "each member shows that member's book count", optionLabel: /Bob/, expected: "2" },
      { name: "favorite shows the favoriteCount prop", optionLabel: /我的最愛/, expected: "4" },
      { name: "hidden shows the hiddenCount prop", optionLabel: /隱藏的書/, expected: "3" },
    ];
  ...
  describe("selection", () => {
    const cases: Array<{ name: string; optionLabel: RegExp; expected: string }> = [
      { name: "所有人的書 → 'all'", optionLabel: /所有人的書/, expected: "all" },
      { name: "其他家人的書 → 'all-except-self'", optionLabel: /其他家人的書/, expected: "all-except-self" },
      { name: "自己的書 → userId", optionLabel: /自己的書/, expected: "user-self" },
      { name: "a member → that member's userId", optionLabel: /Alice/, expected: "user-a" },
      { name: "我的最愛 → favorite sentinel", optionLabel: /我的最愛/, expected: FAVORITE_FILTER_VALUE },
      { name: "隱藏的書 → hidden sentinel", optionLabel: /隱藏的書/, expected: HIDDEN_FILTER_VALUE },
    ];

    it.each(cases)("selecting %s calls onChange with the right value", ({ optionLabel, expected }) => {
    ...
    it("closes the menu after an option is selected", () => {
      renderDropdown();
      const listbox = openMenu();
      fireEvent.click(within(listbox).getByRole("option", { name: /所有人的書/ }));
      expect(screen.queryByRole("listbox")).not.toBeInTheDocument();
    });
  });
