...
import type { ApiResponse, BookEntry, FamilyBookshelf } from "@/api/types";
 ...
 *     baseline). Survivors are kept by SPREAD, never rebuilt.
 ...
 *
 * The same case tables live in `pwa/tests/unit/api/bookshelf-client.test.ts`.
 * Layer 1 is the SHARED implementation both apps import, so the mirrored tables
 * prove each app's own COMPOSITION of the two layers still holds — that part
 * stays per-app.
 */
describe("ApiClient getFamilyBookshelf", () => {
  ...
  describe("request wiring", () => {
    it("sends GET to /api/family/:id/bookshelf and returns the bookshelf envelope", async () => {
      const bookshelf = { members: [makeMember()] };
      globalThis.fetch = mockFetchSuccess(bookshelf);
      ...
      expect(call[0]).toBe(
        `${MOCK_ENDPOINT}/api/family/${FAMILY_ID}/bookshelf`,
      );
  ...
  describe("payload validation", () => {
    ...
    async function sanitizedMembers(
      ...
      const bookshelf = await sanitizedBookshelf({ members });
    ...
    describe("envelope passthrough", () => {
      ...
      it("still validates when error is null, because callers read error as truthy", async () => {
        ...
        globalThis.fetch = mockFetchEnvelope({
          data: { members: [memberWith("userId", 42), makeMember()] },
          error: null,
        });
    ...
    describe("malformed container", () => {
      it.each(NON_ARRAY_CONTAINERS)(
        ...
        async ({ value }) => {
          const bookshelf = await sanitizedBookshelf({ members: value });
      ...
      it.each(NON_RECORD_DATA)(
        ...
        async ({ data }) => {
          ...
          // A non-record `data` carries no bookshelf field at all, so it
          // degrades to the renderable EMPTY state rather than throwing one
          // render later.
    ...
    describe("member preservation", () => {
      ...
      it.each([
        ...
        // `null` is a MEANINGFUL tri-state on the PWA ("never synced"), so it
        // must survive as `null` rather than being dropped or blanked.
        { name: "null", value: null },
      ])(
        "keeps a PWA-shaped member's lastUpdated when it is $name",
        async ({ value }) => {
          const member = await sanitizedMember({
            ...makeMember(),
            lastUpdated: value,
          ...
          expect(asRecord(member).lastUpdated).toBe(value);
          expect(warnSpy).not.toHaveBeenCalled();
        },
      );

      it("passes every other top-level field through by spread while rebuilding members", async () => {
        // `familyId` is the PWA's own top-level field and nothing here owns it;
        // an unknown future one has to survive the same way.
        ...
        expect(asRecord(bookshelf).familyId).toBe(FAMILY_ID);
    ...
    describe("aggregate warnings", () => {
      ...
      it("keeps the container warning and the dropped-count warning mutually exclusive", async () => {
        await sanitizedBookshelf({ members: "nope" });
  ...
  describe("prototype safety", () => {
    ...
    it("does not apply a member's JSON-supplied __proto__", () => {
      const hostile: unknown = JSON.parse(
        `{"members":[{"userId":"${USER_B}","displayName":"Hostile","books":[],"__proto__":{"polluted":"yes"}}]}`,
      );
      ...
      expect(asRecord(member).polluted).toBeUndefined();
    ...
    it("does not apply a book's JSON-supplied __proto__", () => {
      const hostile: unknown = JSON.parse(
        `{"members":[{"userId":"${USER_B}","displayName":"Hostile","books":[{"bookId":"${BOOK_A}","__proto__":{"polluted":"yes"}}]}]}`,
      );
