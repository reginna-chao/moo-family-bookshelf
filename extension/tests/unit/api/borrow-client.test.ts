...
describe("ApiClient borrow methods", () => {
   ...
   * helper. The same case tables live in `pwa/tests/unit/api/borrow-client.test.ts`
   * — the sanitizer itself is the shared implementation in
   * `shared/src/borrow/validation.ts` that both apps import, so the mirrored
   * tables no longer guard two copies against each other; they prove each app's
   * own client still wires that implementation in.
   */
  describe("listBorrowRequests payload validation", () => {
    ...
    async function listPayload(payload: unknown): Promise<BorrowRequest[]> {
      globalThis.fetch = mockFetchSuccess(payload);
