 ...
 * consumer. The fixtures below stay full Extension entity types on purpose —
 * that also pins the EXTENSION's own types as assignable to the shared shapes,
 * which is the drift this file exists to catch. `pwa/tests/unit/entityText.test.ts`
 * does the same for the PWA's types (and covers `sanitizePublicShelfDataText`,
 * which only the PWA client wires).
 *
...
function describeEntitySanitizer<T extends object>(
  ...
  describe(name, () => {
    ...
    if (nullableTextFields.length > 0) {
      ...
      // `null` carries meaning here ("uses the default endpoint"), so the
      // tri-state must survive with exactly the three values it had.
...
describeEntitySanitizer<OtpInfo>("sanitizeOtpInfoText", {
  sanitize: sanitizeOtpInfoText,
  valid: VALID_OTP,
  textFields: ["code"],
  untouchedFields: ["expiresAt"],
});
...
describe("nested collections", () => {
   ...
   * (`extension/src/dialog/MemberList.tsx:82`) readable when the backend omits
   * the key entirely.
   */
 ...
 * regression rows in `extension/tests/unit/api/sanitizeEnvelope.test.ts` assert
