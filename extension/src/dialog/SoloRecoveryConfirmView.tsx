...
export function SoloRecoveryConfirmView({
  ...
  const confirmClass = isLoading
    ? "moo-solo-recovery__confirm moo-solo-recovery__confirm--loading"
    : "moo-solo-recovery__confirm";
  return (
    <div className="moo-onboarding-view">
      ...
      <button type="button" onClick={onConfirm} disabled={isLoading} className={confirmClass}>
        {isLoading ? "處理中..." : "確認重新同步"}
      </button>
      <button type="button" onClick={onBack} className="moo-solo-recovery__back">
        返回
      </button>
    </div>
  );
