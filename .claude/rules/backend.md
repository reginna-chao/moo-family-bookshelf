...
| `ratelimit:user:{scope}:{user_id}:{bucket}` | Per-userId request counter (hourly scopes: `verify`, `verify-write`, `put-books`, `family-prefs`, `public-shelf`; per-minute scopes: `bookshelf`, `borrow-create`, `borrow-list`, `borrow-update`) | 2 × window (hourly: 7200s; per-minute: 120s) |
