...
4. If the change adds or edits any string a user will read — UI labels, banners, error messages, empty states, `site/index.html`, a `CHANGELOG.md` bullet — → also `Read .claude/rules/user-facing-copy.md`. Implementation vocabulary (淨化 / 邊界 / 快照 / 端點 / 降級 …) must not reach the screen. A CHANGELOG bullet goes into the `## 未釋出` section at the top of the file (that rule's "Where a change is recorded"), never into a `## vX.Y.Z` entry — those are already released.
