...
4. If the change adds or edits any string a user will read — UI labels, banners, error messages, empty states, `site/index.html` — OR changes any behaviour a user can observe (a `feat:` / `fix:` / `perf:` / `security:` change, even one with no UI string, e.g. a Worker-only permission fix) → also `Read .claude/rules/user-facing-copy.md`. Implementation vocabulary (淨化 / 邊界 / 快照 / 端點 / 降級 …) must not reach the screen. An observable change MUST ship with a `CHANGELOG.md` bullet in the same change set, written into the `## 未釋出` section at the top of the file (that rule's "Where a change is recorded"), never into a `## vX.Y.Z` entry — those are already released. A change nothing a reader can observe (that rule's Rule 2) gets no bullet.
