...
5. **Diff hygiene:** `git status --short` over the whole tree — the set to stage must equal this feature's expected file list (a retro report already written mid-run under `.claude/reports/` IS on that list). Stray entries (formatter churn, EOL rewrites, another task's leftovers) are inspected and restored, never swept into the commit.
