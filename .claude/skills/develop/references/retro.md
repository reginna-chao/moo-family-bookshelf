...
Lazy-loaded by `/develop` when the user answers YES to the retro question that rides in the
commit-gate AskUserQuestion batch (code-cycle Phase 7 / design Deliver), or asks for a retro
mid-run. Runs in the **main session** — it needs the full conversation history (dispatches,
gates, Fix Cycle rounds, user corrections) that an isolated subagent cannot see. Read this
only after that yes; never load it preemptively, never auto-run, and never ask a second time
in the same run.
