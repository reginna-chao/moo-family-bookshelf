...
| 舊資料殘留的非白名單封面 URL        | 封面 URL 白名單（`sanitizeCoverUrl`）在寫入邊界生效，所有 `public:` 快照經 `buildSnapshot` 收斂；既有資料不主動回填               | 洗淨綁在「該帳號的下一次真正寫入」（書單 PUT 同步、有效的 PATCH、或家庭書櫃偏好儲存）之上，休眠帳號的舊值因此原地保留——主動回填得掃過全部 `user:` key，成本與風險都高於效益。修復前已鑄出的 `public:{share_token}` 快照同樣維持原樣，直到下次刷新或 TTL 到期（永久快照沒有到期日），這段期間由 PWA 的 CSP `img-src` 白名單在讀取端中和：`pwa/public/_headers` 的 `img-src` 已收斂到 Readmoo 網域，即使快照裡留著外部網址，瀏覽器也不會發出該請求。家庭書櫃聚合的讀取端也經 `sanitizeCoverUrl` 消毒（與 `buildSnapshot` 對稱），休眠帳號的舊毒值不會經由聚合端點外洩給家庭成員。餘下的殘餘：KV 舊值仍留存至該帳號下次真正寫入；上述三個渲染面（公開頁 CSP、家庭書櫃聚合消毒、快照重建）已中和。尚未涵蓋：修復前建立的借閱紀錄（`borrow:{requestId}.bookCoverUrl`，無 TTL）保留當時複製的毒值，並在擴充功能借閱分頁原樣渲染——該路徑由 borrow 封面白名單分支另行處理，不在本次修復的範圍內 |
