...
| `kicked:{family_id}:{user_id}`  | `{ removedAt, removedBy }`                                  | Owner 移除成員時寫入的墓碑（TTL 6 小時）；存續期間 join 一律拒絕該 userId（`403 MEMBER_REMOVED`）。僅 key 的存在與否有語意，欄位為診斷用 |
...
5. 被移除成員的 client 收到 MEMBER_REMOVED：Extension 清除本地家庭資料回到引導畫面，
   PWA 顯示訊息並登出——不再無聲重試
6. 墓碑過期後，該使用者可持同步碼重新加入（Inv-4 認可的合法再加入路徑；誤移除的補救即為此）
```
...
| **解綁隔離**     | 離開家庭後，其他成員立即無法存取該使用者的書籍；Owner 移除成員後，kicked 墓碑（TTL 6 小時）阻擋被移除者（含其 client 的自動重連）重新加入 |
...
| 被移除成員於墓碑過期後可重新加入    | `kicked:{family_id}:{user_id}` TTL 6 小時；同步碼即 family_id，無法旋轉                                 | 同步碼與 family_id 同體，旋轉等同重建家庭（波及所有 KV key 與全體成員的 client 儲存）。墓碑已封死被證實的攻擊路徑「無聲自動重加入」：更新後的 client 收到 `403 MEMBER_REMOVED` 即清除本地家庭資料、不再重試；TTL 過後的重新加入需人為持同步碼操作，而這正是 Inv-4 認可的合法再加入路徑（也是誤移除的補救管道）。已知殘餘：未更新的舊版 client 視 MEMBER_REMOVED 為暫時錯誤而持續重試，墓碑過期後仍可能自動補位；KV 跨 colo ~60 秒傳播窗內，與移除同時發生的 join 可能搶在墓碑可見之前成功。若要永久排除某人，唯一手段是解散重建家庭 |
