...
2. DELETE /api/family/:id/member/:uid → 先結算借閱紀錄，再寫入 kicked:{family_id}:{user_id}
   墓碑（TTL 6 小時；寫入失敗 fail-open 並記 log，不擋移除本身），然後刪除 member:{uid}
   反向查詢與 auth token，最後寫入新的成員名單
3. 墓碑刻意排在刪除反向查詢之前：加入家庭先讀反向查詢、後查墓碑，因此看到反向查詢已刪的
   加入請求必定也看到墓碑，回 403 MEMBER_REMOVED，不會把反向查詢補回來（同一資料中心內成立；
   跨資料中心的傳播延遲見「已接受的殘餘風險」）。代價：若寫入墓碑之後的步驟失敗（回 500），
...
| 家庭多鍵寫入沒有交易：自行離開若在撤銷存取之後、更新成員名單之前失敗，會留下「仍在名單上但已無法存取」的成員           | KV 沒有交易，建立、加入、移除成員與解散家庭都要寫不只一把 key。寫入順序刻意安排成「先寫重試能偵測並修補的那一邊」：建立先寫 `member:{userId}` 再寫 `family:{id}`，解散先刪 `family:{id}` 再刪歸屬紀錄與 auth token，兩者半途失敗都只會留下指向不存在家庭的孤兒歸屬紀錄；加入先寫 `family:{id}` 再寫歸屬紀錄，半途失敗後重試會走既有成員重連分支，把歸屬紀錄補回來；移除成員時（owner 踢人）先寫墓碑，再讀目標的歸屬紀錄，指向本家庭才刪除它與 auth token，最後才寫入新的成員名單（自行離開不寫墓碑，其餘順序相同）。建立與加入把「指向的家庭不存在，或名單上已沒有自己」的歸屬紀錄都視為失效（建立會在閘門之後清掉，加入視同沒有家庭而放行、成功時覆寫），`POST /api/auth/lookup` 也套用同一條規則，對這種歸屬紀錄回報沒有家庭。書櫃與成員清單的讀取除了比對歸屬紀錄，還要求呼叫者列在 `family:{id}` 的名單上，歸屬紀錄不再是唯一依據，所以移除半途失敗時目標已經讀不到任何家庭資料，owner 仍看得到他，重試即可收斂。「被踢的人不會連同補回的歸屬紀錄一起回到名單」靠的是兩端對稱的寫入順序：踢除先寫墓碑、再讀目標的歸屬紀錄；加入在寫入歸屬紀錄（重連補回或新成員加入）之後，再查一次墓碑，查到就刪掉自己剛寫的歸屬紀錄，回 `403 MEMBER_REMOVED`，不換發 token。兩端都是先寫自己那把 key、再讀對方那把，所以至少有一端會看到另一端：踢除讀到補回的歸屬紀錄而刪掉它，或加入看到墓碑而撤回。只靠踢除端的順序不夠：目標若原本就沒有歸屬紀錄（先前自行離開或加入半途失敗），加入可能在墓碑寫入前通過墓碑檢查，踢除讀到空的歸屬紀錄而什麼都不刪，補回的歸屬紀錄隨後才寫入，再碰上一筆以踢除前名單覆寫的寫入，就等於繞過墓碑重新加入。補回歸屬紀錄的那次重連另外一律不寫回成員名單（暱稱變更留到下次重連），作為第二道防線。這個保證只在同一資料中心內成立，仍有兩個例外：跨資料中心時 KV 各把 key 各自傳播（約 60 秒）；墓碑寫入失敗（fail-open）會讓那一次踢除失去這層保護。這兩種情況下上述競速仍可能發生。移除時讀到過時的歸屬紀錄而漏刪，會留下指向本家庭、但名單上已沒有他的歸屬紀錄：書櫃、成員清單與借閱路由都會比對名單而拒絕，owner 再踢一次（走 `404 MEMBER_NOT_FOUND` 分支，先補墓碑再刪歸屬紀錄）就會刪掉它；`POST /api/auth/refresh` 只看歸屬紀錄、不讀名單，仍可能替這種殘留換發 token，但那個 token 讀不到任何家庭範圍的資料。仍存在的是 #213 之前就有的 no-CAS 覆寫：`family:{id}` 是整筆讀改寫、沒有 CAS，踢除會與任何同時進行的整筆寫入競速——歸屬紀錄完好的重連帶著新暱稱寫回名單，或本人呼叫暱稱、成員設定端點，只要在墓碑寫入前就讀了名單，都可能在 owner 寫入之後把被踢的成員重新列回名單。他沒有歸屬紀錄，所以家庭書櫃與成員清單仍讀不到；但若那次重連在撤銷之前讀到歸屬紀錄，會替他換發新的 token，而借閱路由（`worker/src/routes/borrow.ts` 的建立、列表、PATCH）與成員設定只檢查成員名單，這個空殼成員因此能讀取家庭的借閱清單、發出借閱請求，他開放分享的書也會出現在其他成員的家庭書櫃裡，直到 owner 再踢一次為止。另開 issue 追蹤（#222）。剩下的半狀態只有自行離開：撤銷之後、寫名單之前失敗時，本人的 token 已刪，無法自己重試                                                                                                                                                                                                                                                             | 恢復途徑有兩條：owner 把他移除（歸屬紀錄已不指向本家庭，重試只會寫名單與墓碑），或本人用同步碼重新加入——重連分支會補回歸屬紀錄——再離開一次。在那之前他佔用一個成員名額、名字留在名單上，但讀不到任何家庭資料，不構成外洩。舊做法（各處的多筆寫入平行送出）可能留下更糟的半狀態：指向不再列出目標的家庭的歸屬紀錄（被移除者仍能換發 token 並讀取家庭書櫃），或無人指向、永遠無法清除的 `family:{id}`。要真正消除得改用 Durable Objects 序列化寫入，代價與「速率限制不是硬上限」那一列相同；已於 #213 改為目前的寫入順序（踢除時墓碑最先寫入），並在讀取端加上名單檢查                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
