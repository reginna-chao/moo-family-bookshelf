...
2. DELETE /api/family/:id/member/:uid → 先結算借閱紀錄，再寫入 kicked:{family_id}:{user_id}
   墓碑（TTL 6 小時；寫入失敗 fail-open 並記 log，不擋移除本身），接著寫入新的成員名單，
   最後才刪除 member:{uid} 反向查詢與 auth token
3. 墓碑刻意排在讀取與刪除反向查詢之前：加入家庭先讀反向查詢、後查墓碑，因此看到反向查詢已刪的
   加入請求必定也看到墓碑，回 403 MEMBER_REMOVED，不會把反向查詢補回來（同一資料中心內成立；
   跨資料中心的傳播延遲見「已接受的殘餘風險」）。成員名單排在刪除反向查詢之前：名單一寫入，
   移除就已生效，之後刪除反向查詢若失敗，只會留下一筆讀不到任何家庭資料的反向查詢，
   任何一次重試都會清掉它。代價：若寫入墓碑之後、名單寫入失敗（回 500），
   目標仍在名單上、卻最多 6 小時無法重連；owner 看得到他，重試移除即可收斂，或以步驟 6 解除。
   若前次移除的墓碑寫入失敗（成員已不在名單、墓碑未寫入），owner 重試移除會走
   404 MEMBER_NOT_FOUND——該分支對「owner 指向他人」會先等冪補寫墓碑、再清掉殘留的反向查詢，
   然後回 404，確保禁令補得上
...
| 速率限制不是硬上限                                                                                                     | **每分鐘**的限制（per-IP 三層 60／10／3，以及 per-userId 的 `bookshelf` 30、`borrow-create` 10、`borrow-list` 60、`borrow-update` 30）改由 Cloudflare 原生 Rate Limiting binding 計數，零 KV 操作；官方明示它是「在單一 Cloudflare 節點內計算、eventually consistent、permissive by design」，因此依然不是硬上限——失準的來源只是從「KV 沒有原子性」換成「分節點且最終一致」。**每小時**的 per-userId 計數器（`verify`、`verify-write`、`put-books`、`family-prefs`、`family-write`、`public-shelf`）與以來源為 key 的呼叫端鎖定仍是 KV get-then-put、無序列化，並發放大倍數取決於攻擊者的併發度；自建者若未在 `wrangler.toml` 加上 binding，每分鐘的限制也退回同一條 KV 路徑（每道每分鐘的限流檢查各輸出一行 `RATE_LIMIT_BINDING_MISSING`；家庭書櫃與借閱路由同時受 per-IP 與 per-userId 兩道檢查，因此單一請求最多兩行）。限額數字與分桶完全不變，敏感層仍是同一個限額下的兩個獨立計數器                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 先前不採用 binding 的理由（eventually consistent、僅在單一節點內生效、因此不是硬上限）依然成立，現在轉為**明示接受**的姿態：既然原本的 KV 計數器同樣不是硬上限，就沒有理由為同一個強度每個請求多付一次 KV 讀＋一次 KV 寫（#160 的讀取放大）。硬上限仍需 Durable Objects，而 DO 會讓每個自建者的 `wrangler.toml` 與升級流程永久多一層 migration，自建能力是本專案的明確賣點。要在邊緣取得真正的強制力，作法是下方「運維層的補強」的 WAF Rate Limiting 規則                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| borrow 紀錄無 TTL：已完成的每人只留最近 50 筆，進行中的長期保留於 KV；成員離開只清掉他自己借的那一側，對手方那一側續存 | 紀錄含當事人的 userId（email 推導、跨家庭可連結）、暱稱、書名與封面 URL。`GET /api/family/:id/borrow` 已過濾為僅回傳呼叫者為 borrower / owner 的紀錄，因此前成員資料僅交易對手方（經 API）與 KV 操作者可見——而且成員離開家庭時（被移除、自行離開或刪除帳號）`settleDepartingBorrower` 會把「他自己借書」那一側的終態紀錄（已歸還／已拒絕／已取消）連同各自的 `borrow:{requestId}` 指標一併移出索引，家庭裡因此只剩下「別人向他借書」那一側的紀錄與尚未歸還的 LENT；`PATCH /api/borrow/:requestId` 對未知 requestId 回 404、對已知但非當事人回 403，雖構成存在性 oracle，但無實務可達的揭露面（requestId 為 `crypto.randomUUID()`，約 122 bit，且 `borrow-update` 每 userId 30 次/分）。借閱索引 `borrows:family:{id}` 現在是單一真相來源（完整 `BorrowRequest[]`），`borrow:{requestId}` 退化為只含 `familyId` 的指標；每次寫入都會修剪：PENDING／LENT 全數保留，終態（已歸還／已拒絕／已取消）依 `borrowerId` 分組，每人只留 `updatedAt` 最新的 `BORROW_HISTORY_KEEP`（50，定義在 `shared/src/borrow/history.ts`，由 `worker/src/kv/schema.ts` 再匯出；同一個模組也提供兩則借閱分頁用的提示文案，UI 顯示的數字與後端修剪的數字因此是同一個值）筆，被淘汰者離開索引並刪除其 `borrow:{requestId}` key（該刪除為 fail-open：失敗只記錄 `BORROW_POINTER_DELETE_FAILED`，不會讓已經寫入成功的索引變成 500），因此「無 TTL 長期保留」現在只對進行中的紀錄與每人最近 50 筆歷史成立。分組在 borrower 而非家庭，是為了讓成員只擠得掉自己的舊紀錄而非家人的，家庭層級的界因此是「成員數 × 50」筆終態加上全部活紀錄。而「成員數」本身也必須有界，否則這個上限等於沒有：borrowerId 可以無限製造（安全發現 F-1——持有同步碼者以全新 userId 加入、開滿待處理申請、離開，那些申請變成已取消紀錄，但其 borrowerId 從此不再寫入，該分組因此永遠輪不到修剪，如此反覆），所以離開時的清除不是附加功能，而是這個界成立的前提。索引另有兩道在建立端的界：同一借閱者的待處理申請上限 `BORROW_MAX_PENDING_PER_BORROWER`（20，超過回 `409 TOO_MANY_PENDING_REQUESTS` 且不寫入——PENDING 依設計不被修剪，是單一成員唯一能無界成長的部分），以及欄位長度上限（`bookId`／`bookTitle` ≤ 200、`bookAuthor` ≤ 100、非空 `bookCoverUrl` ≤ 2048 字元，超過回 `400 INVALID_FIELDS`，與型別檢查同一關卡、在計費之前），兩者都以呼叫者自己的資料計算，第三方無法代為消耗（Inv-6）。家庭解散（單人 owner 離開，或單人 owner 刪除帳號）改為呼叫 `deleteBorrowIndex`：先 fail-open 刪掉索引裡每筆紀錄的 `borrow:{requestId}` 指標，再刪掉 `borrows:family:{id}` 本身，且都排在刪除 family key 之前，因此解散不再留下無人會再經過的孤兒索引——只有清理本身失敗（fail-open，記 `BORROW_POINTER_DELETE_FAILED`／`BORROW_INDEX_DELETE_FAILED`）時才殘留，此時剩下的孤兒指標沒有任何讀取端看得到；`generateFamilyId()` 在 create 前不查 `family:{id}` 是否已存在，約 41 bit 空間又帶 modulo 偏差，familyId 的唯一性是機率保證（生日界約 170 萬個家庭）而非制度保證。帳號刪除（`DELETE /api/user/:id`）除了 user、publicshelves、member、auth token 與公開快照，也走同一組清理：家庭還有其他成員時呼叫 `settleDepartingBorrower`，自己是單人 owner 時呼叫 `deleteBorrowIndex`，兩者都 fail-open——刪帳號沒有「什麼都沒發生」這個答案可回，清理失敗只記錄而不阻擋刪除                                                                                                                                                                                                                                                                      | 終態紀錄已由每次寫入的修剪自然淘汰，離開家庭與解散又補上兩條回收路徑，代價是接受下一列的 no-CAS 覆寫取捨；仍未做的是使用者自己的「主動刪除」：它一樣要重寫 borrow index，會撞上同一個 race 而可能弄丟並發建立中的活紀錄，且會摧毀對手方的交易紀錄（「已取消」「已歸還」的訊號隨之消失）。解散與刪帳號留下的孤兒已由 `deleteBorrowIndex` 在同一個請求內清掉，不必再掃過全部 KV key；剩下的只有 fail-open 清理失敗留下的孤兒指標，沒有讀取端看得到，代價是一把浪費的 key。殘留資料僅交易對手方（經 API）與 KV 操作者可見，與上方安全設計表「儲存安全：明文 JSON + auth token 控管」的既有姿態一致。為 familyId 加碰撞檢查需在每次 create 多一次 KV 讀取與重試迴圈，在 170 萬個家庭的量級之前不划算                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| borrow index 為 read-modify-write，無 CAS：同一家庭的並發寫入會互相覆蓋                                                | `borrows:family:{familyId}` 是單一 KV value，三條寫入路徑——建立借閱、`PATCH /api/borrow/:requestId`、成員離開時的 `settleDepartingBorrower`（移除成員與刪除帳號共用同一支）——都是「讀出整份 → 改 → 寫回」，而 KV 沒有 compare-and-set。同一個家庭在同一瞬間有兩個寫入者時後寫的會覆蓋先寫的，可能弄丟一筆剛建立的借閱、一次狀態變更，或一次自動取消；KV 跨 colo 讀取還可能落後約 60 秒，讓這個窗口比字面上的「同時」更寬。此 race 本身早已存在，但 denormalize 之前它只弄丟索引裡的一個 requestId（紀錄本身另存於 `borrow:{requestId}`），現在弄丟的是整筆紀錄或整次狀態變更。至於「同一個請求內兩次 KV 寫入之間半途失敗」則已不是殘餘：建立借閱改為先寫 `borrow:{requestId}` 指標、再寫索引，修剪則是先寫索引、再刪除被淘汰者的指標（且該刪除 fail-open），兩者方向一致，最壞只會留下孤兒指標——沒有任何讀取端看得到它，對它的 PATCH 與未知 requestId 一樣回 404，重試會以新的 UUID 建立乾淨的紀錄。反方向的「索引裡有、指標沒有」才是要避免的狀態：那是雙方都看得到卻無人能處理的 PENDING 幽靈，修剪不會清掉它（PENDING 免修剪），還會永久卡住同一本書的重新申請                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 家庭規模是兩人到數人，借閱是低頻的手動動作（每一筆都要對方按下同意），兩個寫入者在同一秒撞上同一個家庭的機率極低。撞上時的後果對使用者是可見的（借閱沒出現在清單、狀態沒變），重做一次即可修復；索引是整份覆寫，不會留下寫到一半的索引。要真正消除得改用 Durable Objects（每個家庭一個物件序列化寫入），代價與上方速率限制那一列相同：所有自建者的 `wrangler.toml` 與升級流程永久多一層 migration，而自建能力是本專案的明確賣點。改成 per-borrower 分片索引可縮小碰撞面，但會讓列表與重複檢查退回多次讀取，正是這次改動要移除的成本                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| borrow 當事人 PATCH 與踢除之間的 KV 傳播窗；解散清理失敗後的孤兒紀錄仍由當事人收尾                                     | `PATCH /api/borrow/:requestId` 自 #159 起在解析指標後，與 `borrows:family:{id}` 平行重讀 `family:{id}` 並重驗成員資格：家庭仍存在而呼叫者已不在成員名單時回 `403 NOT_FAMILY_MEMBER`——被踢、自行離開、之後另建家庭取得新 token 都一樣，前成員不再能把舊紀錄的 LENT 單方面改成終態 `RETURNED`（FSM 無任何轉移可離開，也沒有刪除或更正端點）。剩下兩個窗口：（一）KV 跨 colo 傳播約 60 秒，踢除寫入尚未抵達某個 colo 時，前成員在那個 colo 上對舊紀錄發的 PATCH 仍會以踢除前的家庭紀錄通過重驗——最多一次，因為 `RETURNED` 之後 FSM 沒有任何轉移可用；那一次寫入會重寫整份索引（同上一列的 no-CAS 覆寫），但他碰得到的仍只有自己所屬的那一份——`readBorrowPointer` 以 `isValidFamilyId` 驗過指標值才拿去組 key，指標與索引內紀錄的 `familyId` 不一致時一律回 404 且不寫入。（二）家庭解散時 fail-open 的 `deleteBorrowIndex` 失敗（`BORROW_INDEX_DELETE_FAILED`）留下的孤兒索引：`family:{id}` 已不存在，無成員名單可驗，PATCH 因此略過重驗、只比對紀錄自身的 borrowerId / ownerId，讓任一當事人仍能把該筆收尾（非當事人仍回 `403 FORBIDDEN`）。另一側維持對稱：踢除、自行離開與刪除帳號都會呼叫 `settleDepartingBorrower` 取消 PENDING（刪帳號那一側是 fail-open，失敗只記 `BORROW_CLEANUP_FAILED` 並繼續刪除），因此「已刪帳號的 PENDING 仍被對手方核准成 LENT」只在該清理失敗時才可能發生                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 借閱狀態不涉及金流或憑證（前提二），可被竄改的僅是雙方本就都能觸發的終態（`RETURNED` 由 borrower 或 owner 任一方標記），對手方看到的是一筆狀態不如預期的歷史，而非資料外洩，且對手方仍可自行把該筆 LENT 標記為已歸還收尾。傳播窗的寬度是 KV 平台特性（與上一列相同的約 60 秒落後），要消除得改用 Durable Objects，代價與上列相同。孤兒路徑刻意不封：封掉等於把解散清理失敗的後果轉嫁給仍在等對方歸還的當事人，而該路徑只在清理失敗後才存在、又只對紀錄裡的當事人開放。成員資格重驗的代價是每次狀態更新多一次 `family:{id}` 讀取，與索引讀取平行發出，不增加延遲；已於 #159 修正                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 舊資料殘留的非白名單封面 URL                                                                                           | 封面 URL 白名單（`sanitizeCoverUrl`）在寫入邊界生效，判準共四項：https、Readmoo 註冊網域、預設 port，以及 base 不變性（同一字串在「有 base 文件」與「無 base 文件」下必須解析出同一個 href）。第四項擋的是「Worker 驗的是字串、瀏覽器解的是該字串對渲染文件的 base」這道落差：`https:readmoo.com/…` 這類寫法單獨解析時主機看起來是 readmoo.com，實際渲染時卻會落到檢視者自己的 origin。判準是「解析結果是否依賴 base」而非「字面上有沒有 `//`」——`https:\\readmoo.com/x`、`https:/\readmoo.com/x` 這類 base 穩定的寫法在任何瀏覽器都解析成 `https://readmoo.com/x`，屬貨真價實的絕對網址，放行才是正確的。所有 `public:` 快照經 `buildSnapshot` 收斂；既有資料不主動回填                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 洗淨綁在「該帳號的下一次真正寫入」（書單 PUT 同步、有效的 PATCH、或家庭書櫃偏好儲存）之上，休眠帳號的舊值因此原地保留——主動回填得掃過全部 `user:` key，成本與風險都高於效益。修復前已鑄出的 `public:{share_token}` 快照同樣維持原樣，直到下次刷新或 TTL 到期（永久快照沒有到期日），這段期間由公開頁讀取端 `GET /api/public/:shareToken` 也經 `sanitizeCoverUrl` 消毒（回應轉換，零 KV 寫入）中和：舊快照即使留著外部網址，也在送出前被清空。PWA 的 CSP `img-src` 白名單（`pwa/public/_headers` 已收斂到 Readmoo 網域）因此從載重緩解降為**縱深防禦**——`_headers` 僅在支援它的託管（如 Cloudflare Pages）生效，自建於其他靜態主機時沒有 CSP，仍有讀取端消毒兜底。家庭書櫃聚合的讀取端也經 `sanitizeCoverUrl` 消毒（與 `buildSnapshot` 對稱），休眠帳號的舊毒值不會經由聚合端點外洩給家庭成員。餘下的殘餘：KV 舊值仍留存至該帳號下次真正寫入；上述三個渲染面（公開頁讀取端消毒、家庭書櫃聚合消毒、快照重建）已中和。尚未涵蓋：修復前建立的借閱紀錄（`borrow:{requestId}.bookCoverUrl`，無 TTL）保留當時複製的毒值，並在擴充功能借閱分頁原樣渲染——該路徑由 borrow 封面白名單分支另行處理，不在本次修復的範圍內                                                                                                                                                                                                                                                                                                                                                                                             |
| 舊資料殘留的非白名單書籍連結 URL                                                                                       | 書籍連結白名單（`sanitizeReadmooUrl`，與封面共用 `shared/src/config/readmoo.ts` 內同一個判斷核心，因此同樣是 https、Readmoo 註冊網域、預設 port、base 不變性四項條件——判準為「解析是否依賴 base」，不是「有沒有 `//`」，詳見上一列）在寫入邊界生效，所有 `public:` 快照經 `buildSnapshot` 收斂；既有資料不主動回填                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | 與封面同構的一條路徑，差別在渲染面是可點擊的 `<a href>` 而非 `<img src>`：惡意家庭成員繞過 UI 直接呼叫 API，即可把 `readmooUrl` 指向釣魚站或任意第三方網站，受害者點擊後就落在攻擊者控制的內容上，目的地也因此取得其 IP 與 User-Agent。**不會**外洩的是 Referer：四個渲染點的 `<a>` 都同時帶 `target="_blank"` 與 `rel="noopener noreferrer"`，而 `noreferrer` 會完全抑制該標頭——那個屬性在此是載重設計而非樣板，日後若有人想拿掉，請先回到本列。要「使用者點擊」才觸發（不像封面會自行載入），但這降低的是發生率而非嚴重性：點擊正好發生在使用者相信自己要打開 Readmoo 的那一刻。CSP 的 `img-src` 對這一面毫無作用（它從來就不管導航），伺服器端消毒是主要防線。洗淨同樣綁在「該帳號的下一次真正寫入」（書單 PUT 同步、有效的 PATCH、或家庭書櫃偏好儲存）之上，白名單上線前寫入的舊值因此在休眠帳號中原地保留——理由與封面相同：主動回填得掃過全部 `user:` key，成本與風險都高於效益。已鑄出的 `public:{share_token}` 快照同樣維持原樣，直到下次刷新或 TTL 到期（永久快照沒有到期日）。這段期間由兩個讀取端消毒中和：公開頁 `GET /api/public/:shareToken`（回應轉換，零 KV 寫入）與家庭書櫃聚合 `GET /api/family/:id/bookshelf`，兩者皆再經 `sanitizeReadmooUrl`，因此休眠帳號的舊毒值不會經由這兩個面外洩。渲染端另有兜底：Extension 與 PWA 在 `<a href>` 前以 `safeBookUrl` 再過濾一次（與既有的 `safeCoverUrl` 對稱），所以即使使用者連的是尚未更新的自建 Worker，連結也不會指向站外。餘下的殘餘：KV 舊值仍留存至該帳號下次真正寫入。借閱紀錄（`borrow:{requestId}`）不含書籍連結欄位，不受此路徑影響 |
| 家庭多鍵寫入沒有交易：移除成員若在寫入名單之後、刪除歸屬紀錄之前失敗，會留下一筆讀不到任何資料的歸屬紀錄               | KV 沒有交易，建立、加入、移除成員與解散家庭都要寫不只一把 key。寫入順序刻意安排成「先寫重試能偵測並修補的那一邊」：建立先寫 `member:{userId}` 再寫 `family:{id}`，解散先刪 `family:{id}` 再刪歸屬紀錄與 auth token，兩者半途失敗都只會留下指向不存在家庭的孤兒歸屬紀錄；加入先寫 `family:{id}` 再寫歸屬紀錄，半途失敗後重試會走既有成員重連分支，把歸屬紀錄補回來；移除成員時（owner 踢人）先寫墓碑，再寫入新的成員名單，最後讀目標的歸屬紀錄，指向本家庭才刪除它與 auth token（自行離開不寫墓碑，其餘順序相同）。建立與加入把「指向的家庭不存在，或名單上已沒有自己」的歸屬紀錄都視為失效（建立會在閘門之後清掉，加入視同沒有家庭而放行、成功時覆寫），`POST /api/auth/lookup` 也套用同一條規則，對這種歸屬紀錄回報沒有家庭。書櫃與成員清單的讀取除了比對歸屬紀錄，還要求呼叫者列在 `family:{id}` 的名單上（借閱路由本來就只看名單），歸屬紀錄不再是唯一依據。名單因此排在刪除歸屬紀錄之前寫入：名單一寫入，移除就已生效，家庭書櫃也不再收錄他分享的書；之後刪除歸屬紀錄若失敗，只會留下一筆指向本家庭、名單上卻已沒有他的歸屬紀錄（連同他的 auth token），讀不到任何家庭資料，建立、加入與 lookup 也都把它視為失效。任何一次重試都會清掉它：他已不在名單上，owner 再踢一次或本人再按一次離開，都會走 `404 MEMBER_NOT_FOUND` 分支，把這筆歸屬紀錄連同 token 一起刪除。若刪除只成功了 token 那一半，本人無法再重試離開，但殘留的歸屬紀錄同樣讀不到資料，也不擋他建立或加入家庭。名單寫入本身失敗時（踢人的墓碑已寫入），目標仍在名單上、最多 6 小時無法重連，owner 看得到他，重試即可收斂。「被踢的人不會連同補回的歸屬紀錄一起回到名單」靠的是兩端對稱的寫入順序：踢除先寫墓碑、再讀目標的歸屬紀錄；加入在寫入歸屬紀錄（重連補回或新成員加入）之後，再查一次墓碑，查到就刪掉自己剛寫的歸屬紀錄，回 `403 MEMBER_REMOVED`，不換發 token。踢除在這兩步之間寫入名單，不影響這個配對。兩端都是先寫自己那把 key、再讀對方那把，所以至少有一端會看到另一端：踢除讀到補回的歸屬紀錄而刪掉它，或加入看到墓碑而撤回。只靠踢除端的順序不夠：目標若原本就在名單上卻沒有歸屬紀錄（先前加入半途失敗），加入可能在墓碑寫入前通過墓碑檢查，踢除讀到空的歸屬紀錄而什麼都不刪，補回的歸屬紀錄隨後才寫入，再碰上一筆以踢除前名單覆寫的寫入，就等於繞過墓碑重新加入。補回歸屬紀錄的那次重連另外一律不寫回成員名單（暱稱變更留到下次重連），作為第二道防線。這個保證只在同一資料中心內成立，仍有兩個例外：跨資料中心時 KV 各把 key 各自傳播（約 60 秒）；墓碑寫入失敗（fail-open）會讓那一次踢除失去這層保護。這兩種情況下上述競速仍可能發生。移除時讀到過時的歸屬紀錄而漏刪，留下的是同一種指向本家庭、但名單上已沒有他的歸屬紀錄：書櫃、成員清單與借閱路由都會比對名單而拒絕，owner 再踢一次（走 `404 MEMBER_NOT_FOUND` 分支，先補墓碑再刪歸屬紀錄）就會刪掉它；`POST /api/auth/refresh` 只看歸屬紀錄、不讀名單，仍可能替這種殘留換發 token，但那個 token 讀不到任何家庭範圍的資料。仍存在的是 #213 之前就有的 no-CAS 覆寫：`family:{id}` 是整筆讀改寫、沒有 CAS，踢除會與任何同時進行的整筆寫入競速——歸屬紀錄完好的重連帶著新暱稱寫回名單，或本人呼叫暱稱、成員設定端點，只要在墓碑寫入前就讀了名單，都可能在 owner 寫入之後把被踢的成員重新列回名單。他沒有歸屬紀錄，所以家庭書櫃與成員清單仍讀不到；但若那次重連在撤銷之前讀到歸屬紀錄，會替他換發新的 token，而借閱路由（`worker/src/routes/borrow.ts` 的建立、列表、PATCH）與成員設定只檢查成員名單，這個空殼成員因此能讀取家庭的借閱清單、發出借閱請求，他開放分享的書也會出現在其他成員的家庭書櫃裡，直到 owner 再踢一次為止。另開 issue 追蹤（#222） | 名單寫入之後殘留的歸屬紀錄讀不到任何家庭資料、不佔成員名額，也不擋本人建立或加入家庭，不構成外洩；任何一次重試（owner 再踢一次，或本人再按一次離開）都會經 `404 MEMBER_NOT_FOUND` 分支把它清掉。反過來的順序（先刪歸屬紀錄、後寫名單）在寫名單失敗時會留下更糟的狀態：目標仍在名單上，他分享的書繼續出現在其他成員的家庭書櫃；自行離開時本人的 token 已刪，無法自己重試。更早的做法（各處的多筆寫入平行送出）可能留下同一種歸屬紀錄，但當時讀取端只看歸屬紀錄，被移除者因此仍能換發 token 並讀取家庭書櫃；也可能留下無人指向、永遠無法清除的 `family:{id}`。要真正消除得改用 Durable Objects 序列化寫入，代價與「速率限制不是硬上限」那一列相同；已於 #213 改為目前的寫入順序（踢除時墓碑最先寫入，成員名單先於刪除歸屬紀錄），並在讀取端加上名單檢查                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
`worker/src/middleware/kvOpCounting.ts` 為每個 `/api/*` 請求輸出一行結構化 log：`{ event: "kv_ops", method, route, status, reads, writes, deletes }`。計數方式是把 `c.env.KV` 換成一層純轉發的 Proxy——只累加 `get` / `getWithMetadata`（reads）、`put`（writes）、`delete`（deletes），不快取、不合併、不改變任何呼叫語意。它註冊在 `/api/*` 上，且刻意排在 `rateLimit`、`authMiddleware` **之前**，因此兩道限流計數器**仍走 KV 的部分**、以及 auth token 的 get，都與 handler 自身的操作記在同一行——那份固定成本正是需要被看見的部分。（每分鐘的限制改用原生 Rate Limiting binding 之後已不再產生 KV 操作，剩下的只有每小時的 per-userId scope，以及自建者未設定 binding 時的退回路徑。）
...
- **驗證閘門涵蓋三個公開端點**：`POST /api/family`（建立家庭）、`POST /api/family/:id/join`（加入家庭）、`POST /api/auth/lookup`（查詢所屬家庭）。三者共用同一個 `validateVerification`，錯誤碼與鎖定行為完全一致（`400 INVALID_VERIFY_SECRET` / `403 VERIFICATION_REQUIRED` / `403 VERIFICATION_FAILED` / `429 VERIFICATION_LOCKED` / `429 RATE_LIMITED`，兩種 429 皆附 `retryAfter`）。原因是 userId 由 email 加固定鹽推導而來、屬公開可猜的識別碼，而個人設定 `user:{userId}` 在離開家庭後仍會保留（設計如此）：
  ...
  - **`ALREADY_IN_FAMILY`（409）排在閘門之前，是刻意保留的極小揭露**：它等於告訴未通過驗證的呼叫方一件布林事實——「這個 Email 對應的帳號目前屬於某個仍存在、且名單上有他的家庭」（不含是哪一個）。歸屬紀錄 `member:{userId}` 若指向已不存在的 `family:{id}`（建立或解散半途失敗留下的孤兒），或指向名單上已沒有他的家庭（加入與踢除交錯留下的殘留），都不算有家庭：建立會在閘門之後清掉它，加入則直接放行、成功時覆寫它，因此揭露的範圍比「有歸屬紀錄」更小。之所以不把閘門提前：這個衝突是「便宜且終局」的，任何密鑰都無法讓該請求成功，先驗證只會白白要求使用者輸入 PIN、消耗該帳號的驗證嘗試額度，最後仍然拒絕。建立家庭與加入家庭採同一順序，行為一致。真正有價值的資訊（familyId／同步碼、auth token、成員資料）與所有寫入都仍在閘門之後
- **`MEMBER_REMOVED`（403）排在閘門之後**：加入家庭在驗證閘門通過後才檢查 kicked 墓碑，「此帳號最近被移出此家庭」這件事實只對通過該帳號驗證的呼叫方（或未設驗證的帳號）揭露，不構成新的 pre-gate 揭露。此檢查刻意同時涵蓋 QR token bypass（開踢前幾分鐘鑄造的 QR token 不得凌駕開踢意圖）與「家庭紀錄尚含此成員」的重連分支（墓碑存續期間仍在名單上，只可能是移除仍在進行中（墓碑先寫）、移除在寫入墓碑後失敗，或與移除競速的 stale KV 讀取，一律以較新的移除意圖為準，fail-closed）
...
- 已接受的殘餘風險（非缺陷，權衡後保留）：
  ...
  - **跨來源暴力破解不再由鎖定機制擋下**：從「單一來源」發動的暴力破解仍受 per-IP 敏感端點限制（每分鐘 3 次）約束——**驗證閘門的三個入口（建立／加入／查詢所屬家庭）現已全部列為敏感端點**（`isSensitivePublicRoute()`），因為查詢所屬家庭同樣讓未驗證的呼叫方可以測試密鑰，而且是三者中最便宜的一條（純讀取、前面沒有 409 終局衝突）。分類只看路徑不看 body：速率限制中介層在解析 body 之前就執行，無從得知這次有沒有帶 `verifySecret`，而攻擊者本來就會每次都帶。**敏感端點使用同一個限額（每分鐘 3 次）但分成兩個獨立計數器**：建立／加入家庭記在 `ratelimit:sens:{ip}`，查詢所屬家庭記在 `ratelimit:sens:lookup:{ip}`（兩者都是原生 Rate Limiting binding 上的 key，共用同一個 3 次/分鐘的 binding；未設定 binding 而退回 KV 計數時，key 尾端再接一個分鐘桶）。原因是一次正常的登入流程本身就會在同一分鐘內用掉 3 次敏感請求（先 lookup 探詢、再 lookup 帶密鑰、最後 create／join），若共用一個計數器，使用者只要 PIN 打錯一次要重試、或家中第二個人在同一個 NAT／IPv6 /64 之後接著設定，就會立刻收到 60 秒的 429。拆開之後這段流程是 lookup 桶 2 次、create／join 桶 1 次，重試不再互相排擠。分桶只改「記在哪個 key」，限額與嚴格程度完全不變（`rateLimitBucketFor()` 於 `worker/src/middleware/rateLimit.ts`）。攻擊者輪替來源前綴後，則由 per-userId 的驗證嘗試上限（每小時 10 次失敗，涵蓋同樣三個入口）接手擋下。注意這些限制在 `DEV_MODE=1` 的 Worker 上會被停用（見 `worker/DEPLOY.md`），因此 dev worker 不得存放真實資料
  ...
  - **沒有任何一道限制是硬上限**：每分鐘的限制（per-IP 三層，以及 per-userId 的 `bookshelf`／`borrow-*`）已改用 Cloudflare 原生 Rate Limiting binding，它在單一 Cloudflare 節點內計算、eventually consistent，官方明示 permissive by design。每小時的 per-userId 計數器與驗證嘗試上限則仍是 KV get-then-put、沒有序列化：同時併發送出的請求會讀到同一個計數值而全部放行，超額幅度取決於**攻擊者自己的併發數**，不是固定的 ~2 倍。兩條路徑都只對循序流量成立。要有硬上限必須改用 Durable Objects，屬另案決策，目前不在 backlog 內（`docs/project-plan.md` BE-4 只涵蓋「改用原生 binding」，已完成）；邊緣層則可用上方「運維層的補強」所述的 WAF Rate Limiting 規則
...
| `ratelimit:sens:{ip}:{bucket}`                  | 該分鐘內來自此來源的建立／加入家庭次數（上限 3）。**僅在未設定原生 Rate Limiting binding 時才寫入**；正常部署下這個 key 不再產生 | 120 秒   |
| `ratelimit:sens:lookup:{ip}:{bucket}`           | 該分鐘內來自此來源的查詢所屬家庭次數（上限 3，獨立）。同上，僅退回 KV 時才寫入                                                   | 120 秒   |
