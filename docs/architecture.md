...
| borrow 當事人身分永久有效：被踢除、自行離開或家庭解散後仍可 PATCH 尚存的舊紀錄                                         | `PATCH /api/borrow/:requestId` 僅比對紀錄自身的 borrowerId / ownerId 授權，從不重讀 `family:{id}`、也不重驗成員資格，「當事人」身分因此是永久的。被踢成員的 token 雖已由 `deleteAuthToken` 撤銷，但 `member:{uid}` 同時被刪除、重建家庭不會 409，他可另建自己的家庭取得有效 token 後再對舊紀錄發 PATCH；FSM 中 `RETURNED` 為終態（無任何轉移可離開，也沒有刪除或更正端點），被踢的借閱者因此能把 LENT 單方面改成「已歸還」且不可逆。另一側現在對稱：踢除、自行離開與刪除帳號都會呼叫 `settleDepartingBorrower` 取消 PENDING（刪帳號那一側是 fail-open，失敗只記 `BORROW_CLEANUP_FAILED` 並繼續刪除），因此「已刪帳號的 PENDING 仍被對手方核准成 LENT」只在該清理失敗時才可能發生                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 借閱狀態不涉及金流或憑證（前提二），可被竄改的僅是雙方本就都能觸發的終態（`RETURNED` 由 borrower 或 owner 任一方標記），對手方看到的是一筆狀態不如預期的歷史，而非資料外洩；幽靈使用者本身已無 token，對手方仍可自行把該筆 LENT 標記為已歸還收尾。修正需讓 PATCH 在家庭仍存在時重驗 membership，並同時決定孤兒紀錄該由誰收尾，代價是每次狀態更新多一次 `family:{id}` 讀取；此為目前接受的姿態，修法追蹤於 #159                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 前成員的一次 PATCH 會重寫整份家庭借閱索引                                                                              | 授權語意沿用上一列：`PATCH /api/borrow/:requestId` 只比對紀錄自身的 borrowerId / ownerId，被踢或自行離開的成員因此仍能把自己那筆 LENT 標記為 `RETURNED`——只有這一次，因為 `RETURNED` 是終態，之後 FSM 沒有任何轉移可用。denormalize 之後，這一次寫入不再只改 `borrow:{requestId}` 一把 key，而是「讀出整份 `borrows:family:{id}` → 改一筆 → 整份寫回」，並在寫回時套用修剪。他能碰到的仍只有自己所屬的那一份索引：`readBorrowPointer` 以 `isValidFamilyId` 驗過指標值才拿去組 key，PATCH 又在指標與索引內紀錄的 `familyId` 不一致時一律回 404 且不寫入                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 可觸發的變更面沒有變大：能改的仍只有他本來就是當事人的那一筆，而且只有一次。修剪是 per-borrower 分組，只會淘汰他自己超出 20 筆的舊終態紀錄，碰不到家人的歷史；整份寫回帶來的覆蓋風險等同上一列的 no-CAS race，家庭規模與借閱頻率使兩個寫入者同秒相撞的機率極低，重做一次即可修復。要讓寫入只影響一筆得放棄「單一 KV value 的索引」（等於退回每筆一次讀取，正是這次改動要移除的成本）或改用 Durable Objects，代價與上兩列相同；要根除則需 PATCH 重驗 membership，追蹤於 #159                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
